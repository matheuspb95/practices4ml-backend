...
from app.models.enums import Challenges
...
def get_views_areas(token: str = Depends(oauth2_scheme)):
    payload = decode_token(token)
    views = [{"label": e.value, "value": 0} for e in Challenges]
    for pract in db.practices.find():
...
def get_comments_areas(token: str = Depends(oauth2_scheme)):
    payload = decode_token(token)
    comments = [{"label": e.value, "value": 0} for e in Challenges]
    for pract in db.practices.find():
